/* ============================================================
   ...
   Cart-based, dynamic inventory-aware checkout
   ============================================================ */
...
async function loadMenuStock() {
  try {
    ...
    /*
     * Make sure the customer's cart never contains more
     * of an item than is currently available.
     */
    const cart = getCart();
    let changed = false;
    ...
    for (let i = cart.length - 1; i >= 0; i--) {
      ...
      /*
       * A finite stock value means the dish has a real
       * inventory number.
       */
      if (Number.isFinite(stock) && stock >= 0) {

        /*
         * No stock left — remove the item entirely.
         */
        ...
        /*
         * Some stock remains, but the cart quantity is too high.
         */
  ...
  els.cartItems.innerHTML = cart
    .map(
      (item, i) => {
        ...
        /*
         * If stock is a number, use it.
         * If stock is null/unlimited, retain the existing
         * 8-option fallback used by the menu.
         */
        const stock = menuStock[item.dish];
        ...
        /*
         * This should normally never be necessary because
         * loadMenuStock() already adjusts the cart, but it
         * protects the UI from an invalid selected quantity.
         */
        ...
        return `
          <div style="display:flex; justify-content:space-between; align-items:center; padding:12px 0; border-bottom:1px solid var(--beige-line);">
            ...
            <div style="display:flex; align-items:center; gap:14px;">
              ...
              <select
                onchange="handleQtyChange(${i}, this.value)"
                style="width:64px;"
              >
                ${Array.from(
                  { length: maxQty },
              ...
              <span style="min-width:56px; text-align:right;">
                ₹${item.price * item.qty}
              </span>
              ...
              <button
                ...
                style="background:none; border:none; color:#8a3b3b; cursor:pointer; font-size:0.85rem;"
              >
                Remove
              </button>
    ...
    .map(
      (item) =>
        `<div class="summary-row">
          <span>${item.dish} x${item.qty}</span>
...
async function loadSlots() {
  try {
    ...
    els.slot.innerHTML =
      `<option value="">Couldn't load slots, please refresh</option>`;
  }
}
...
async function loadDates() {
  try {
    ...
    els.date.innerHTML =
      `<option value="">Couldn't load dates, please refresh</option>`;
  }
}
...
els.form.addEventListener("submit", async (e) => {
  ...
  els.payBtn.innerHTML =
    `<span class="spinner"></span> Preparing order…`;

  const itemsSummary = cart
    .map((i) => `${i.dish} x${i.qty}`)
    .join(", ");
  ...
  try {
    ...
    console.warn(
      "Order logging failed, continuing to payment:",
      err
    );
  }
...
(async function initCheckout() {
  ...
  /*
   * Load the current stock first.
   * This also adjusts/removes stale cart quantities
   * before we render the cart.
   */
  await loadMenuStock();
